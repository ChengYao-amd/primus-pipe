...
def get_bubble_ratio(data, iter_time, vpp_size, micro_batches):
    non_bubble_time = 0
    for i in range(vpp_size * micro_batches):
...
def get_task_data(task_list):
    ...
    for task in task_list:
        ...
        vpp_size = task_data["config"]["vpp_size"]
        micro_batches = task_data["config"]["micro_batches"]
        ...
        for iter_idx in task_data["iter_to_vis"]:
            ...
            iter_time = 0
            for pp_rank in range(pp_size):
                iter_time = max(iter_time, pp_rank_dict[pp_rank][str(iter_idx)]["total"])
            ...
            for pp_rank in range(pp_size):
                ...
                bubble_ratio = get_bubble_ratio(
                    iter_dict[iter_idx][pp_rank], iter_time, vpp_size, micro_batches
                )
        ...
        color_dict = get_color_dict(vpp_size)
        task_data["color_dict"] = color_dict

        task_data_list.append(task_data)

        print(f"task {task['title']} generate data done")
...
def draw_pipeline(
    ax, iter_data, pp_size, vpp_size, micro_batches, chunk_info_list, color_dict, draw_wgrad=False
):
    series = ["fwd", "bwd"]
    if draw_wgrad:
        series += ["wgrad"]
    for pp_rank in range(pp_size):
        for serie in series:
            ...
            assert len(iter_data[pp_rank][minibatch_key]) == len(
                chunk_info_list
            ), f"len(iter_data[pp_rank][{minibatch_key}]) != len(chunk_info_list), {len(iter_data[pp_rank][minibatch_key])} != {len(chunk_info_list)}"
...
def get_chunk_info_list(micro_batches, pp_size, vpp_size, color_dict):
    chunk_info_list = []
    id_list = [i + 1 for i in range(micro_batches)]
    color_idx_list = [0 for _ in range(micro_batches)]
    chunk_num = micro_batches // pp_size
    if vpp_size > 1:
        color_idx_list = [[i] * pp_size for i in range(vpp_size)] * chunk_num
        ...
        for i in range(chunk_num, 0, -1):
            tmp_id_list.insert(i, id_list_split[i - 1] * (vpp_size - 1))
        id_list = sum(tmp_id_list, [])
        id_list = sum(tmp_id_list, [])

    for i in range(micro_batches * vpp_size):
...
def draw_sub(axs, task_data, iter_time_max):
    ...
    chunk_info_list = get_chunk_info_list(micro_batches, pp_size, vpp_size, color_dict)
    ...
    for ax_idx, iter_idx in enumerate(task_data["iter_to_vis"]):
        ...
        for i in range(vpp_size):
            fwd_rect = patches.Patch(
                color=color_dict["fwd_rect_color"][i], label="fwd" if vpp_size == 1 else "fwd_vpp_" + str(i)
            )
            bwd_rect = patches.Patch(
                color=color_dict["bwd_rect_color"][i], label="bwd" if vpp_size == 1 else "bwd_vpp_" + str(i)
            )
            ...
            if draw_wgrad:
                wgrad_rect = patches.Patch(
                    ...
                    label="wgrad" if vpp_size == 1 else "wgrad_vpp_" + str(i),
                )
        ...
        draw_pipeline(
            ax, iter_data, pp_size, vpp_size, micro_batches, chunk_info_list, color_dict, draw_wgrad
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="simulation/config/simulation.yaml")
