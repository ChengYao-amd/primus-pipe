...
"""MNIST Training with ZBV Greedy Pipeline Parallelism

ZBV Greedy uses the same V-fold topology as ZBV Formatted but with a greedy
scheduling strategy that can optimize for different memory budgets:
    - "half": allows up to half the peak memory (better bubble ratio)
    - "min":  minimizes peak activation memory

Forward path:  rank0(c0) -> rank1(c0) -> ... -> rankN(c0)
                  -> rankN(c1) -> ... -> rank1(c1) -> rank0(c1)

Usage:
    torchrun --nproc_per_node=4 example/pytorch/zbv_greedy_demo.py
"""
...
from primuspipe.scheduler.algorithms.zbv_greedy import ScheduleZBVGreedy
...
def run_model(pp_size: int, vpp_size: int = 2, memory_config: str = "half"):
    ...
    if is_loss_rank:
        print(f"ZBV Greedy schedule with memory_config='{memory_config}'")
    ...
    for epoch in range(num_epochs):
        for step, (images, labels) in enumerate(train_loader):
            ...
            schedule = ScheduleZBVGreedy(
                pp_size=pp_world_size, vpp_size=vpp_size, micro_batches=n_mb, memory_config=memory_config
            )
...
if __name__ == "__main__":
    """torchrun --nproc_per_node=4 example/pytorch/zbv_greedy_demo.py"""
    run_model(pp_size=4, vpp_size=2, memory_config="half")
