...
"""MNIST Training with Interleaved 1F1B Pipeline Parallelism

Demonstrates training a simple MLP on MNIST using the interleaved 1F1B
pipeline schedule with virtual pipeline parallelism (VPP). Each rank holds
multiple model chunks, reducing pipeline bubble overhead.

With pp_size=4 and vpp_size=2, the forward path traverses 8 virtual stages:
    rank0-c0 -> rank1-c0 -> rank2-c0 -> rank3-c0 ->
    rank0-c1 -> rank1-c1 -> rank2-c1 -> rank3-c1

Usage:
    torchrun --nproc_per_node=4 example/pytorch/1f1b_interleaved_demo.py
"""
...
from primuspipe.scheduler.algorithms.interleaved_1f1b import ScheduleInterleaved1F1B
...
def bind_func_for_scheduler_table(
    ...
    bwd_func = functools.partial(backward_step, dist_manager, conf_dict)
    ...
    for node in my_schedule:
        ...
        elif node.func_type == FuncType.BW:
...
def run_model(pp_size: int, vpp_size: int):
    ...
    conf_dict = {
        ...
        "micro_batches": 16,
        "vpp_size": vpp_size,
        "communication_mode": "async_p2p",
    }

    models = {
        c: create_stage_model(
            pp_rank, pp_world_size, chunk=c, vpp_size=vpp_size, hidden_size=conf_dict["hidden_size"]
        )
    ...
    handler_dict = default_handler_dict
    schedule_runner = ScheduleRunner(handler_dict)
    ...
    for epoch in range(num_epochs):
        for step, (images, labels) in enumerate(train_loader):
            ...
            schedule = ScheduleInterleaved1F1B(pp_size=pp_world_size, vpp_size=vpp_size, micro_batches=n_mb)
            ...
            if step == 0 and epoch == 0 and pp_rank == 0:
                schedule.print_schedule_table(schedule_table, filter=None)
