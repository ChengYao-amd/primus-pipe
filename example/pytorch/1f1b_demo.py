...
"""MNIST Training with 1F1B Pipeline Parallelism

Demonstrates training a simple MLP on MNIST using the basic 1F1B pipeline
schedule. The model is split across pipeline stages, where each rank holds
one linear layer of the MLP.

Usage:
    torchrun --nproc_per_node=4 example/pytorch/1f1b_demo.py
"""
...
from primuspipe.scheduler.algorithms.basic_1f1b import Schedule1F1B
...
def run_model(pp_size: int):
    ...
    model = create_stage_model(pp_rank, pp_world_size, hidden_size=conf_dict["hidden_size"])
    ...
    schedule = Schedule1F1B(pp_size=pp_world_size, vpp_size=1, micro_batches=n_mb)
